...
まとめてバックアップできる機能をスクリプトにつけているためこういう説明になっているが、実際には該当日データのバックアップは1回しかしないよう設定しており、前回バックアップとの比較を配慮する必要はない。<br>

        [図: バケット構造]
        s3://midori-lanscope-logs-archive/
        ├─ csv/YYYY/MM/DD/***.csv (旧バックアップ様式)
        ├─ dat/YYYY/MM/DD/***.dat (旧バックアップ様式)
